  ...
  connection.query(query, [user.email], (err, results) => {
    if (!err) {
      if (results.length <= 0 || results[0].password != user.password) {
        ...
        return res
          .status(401)
          .json({ message: "Please contact Admin for Approval" });
...
//   host: process.env.ETHEREAL_HOST, // Use the Ethereal SMTP host
//   port: process.env.ETHEREAL_PORT, // Use the Ethereal SMTP port
//   auth: {
    ...
    if (!err) {
      if (results.length <= 0) {
        ...
        var mailOptions = {
          ...
          html:
            "<p><b>Your Log in Details for Ringa Cafe Management system</b><br><b>Email: </b>" +
            results[0].email +
            "<br><b>Password: </b>" +
            results[0].password +
...
router.get("/checkToken", auth.authenticateToken, (req, res) => {
  return res.status.json({ message: "true" });
});
